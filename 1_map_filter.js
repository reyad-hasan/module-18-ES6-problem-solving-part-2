...
const applyDiscount = (prices, rate) => {
    const discountApply = prices.map((elem) => elem - (elem * rate) / 100);
    return discountApply
}

const prices = [500, 1000, 250];
const rate = 5
// const result = applyDiscount(prices, rate);
// console.log(result);
...
const getPassingStudents = (students, threshold) => {
    const passingStudents = students.filter((student) => student.marks >= threshold);
